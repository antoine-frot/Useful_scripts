...
if [ "$job_count" -gt 0 ]; then
    ...
    maximal_name_width=$((terminal_width - time_width - 5 - 43)) # -5 for spaces and -28 for the other informations
    ...
    # Finally display
    squeue -u $USER -o "%.8i %.${name_width}j %.${time_width}M %.4C %.7m %.8Q %R"
fi
