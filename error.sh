...
for molecule in ${@:-*}; do
  if [ -d "$molecule" ]; then
    for source_path in "${molecule}/${molecule}-"*; do
      grep -H -e "ORCA finished by error termination" -e "aborting the run" "$source_path"/"${molecule}"*.out 2>/dev/null | awk -F"/" '{print $NF}' | head -n 1
      grep -H "INPUT ERROR" "$source_path"/"${molecule}"*.out 2>/dev/null | awk -F"/" '{print $NF}'
