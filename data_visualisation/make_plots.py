...
def generate_plot_experiment_computed(exp_data: dict, luminescence_type: str, computed_data: dict, methods_optimization: list, 
                                    ...
                                    gauge=None, dissymmetry_variant=None, molecules=None, legend_fontsize=9):
    """
    ...
    for i, molecule in enumerate(molecule_data):
        molecule_markers[molecule] = available_markers[i]
...
def generate_plot_experiment_multiple_computed(exp_data: dict, luminescence_type: str, computed_data: dict, methods_optimization: list, 
    ...
    # Assign unique markers to each molecule
    molecule_markers = {}
    if len(molecules) > len(available_markers):
    ...
    # Add the legends
    first_legend = plt.legend(handles=molecule_handles, loc='lower', 
                                title='Molecules', fontsize=legend_fontsize)
    plt.gca().add_artist(first_legend)
    
    plt.legend(handles=method_handles, loc='upper', 
                title='Methods', fontsize=legend_fontsize)
